/* CardMesh SCREENSHOT TOOL config -- a variant of src/ui/lv_conf.h for
 * compiling+running DashboardScreen natively on the host (macOS), with the
 * Linux framebuffer driver disabled since linux/fb.h isn't available here.
 * Colors/layout otherwise match the real device config (see that file).
 * Keep LV_USE_LINUX_FBDEV/LV_COLOR_DEPTH changes here in sync with
 * src/ui/lv_conf.h if that file's display settings change.
 */
/**
 * @file lv_conf.h
