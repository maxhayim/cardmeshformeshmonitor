/* CardMesh device build config.
 * Based on LVGL's lv_conf_template.h with LV_USE_LINUX_FBDEV / display
 * settings overridden for the CardputerZero target. See
 * docs/DEVICE_BUILD.md for assumptions (color depth, device paths) that
 * have not been verified against real hardware.
 * Global keyboard shortcuts (R/Q) are read directly from /dev/input via
 * EvdevKeyboard rather than LV_USE_EVDEV's focus/group indev system,
 * since the MVP dashboard has no focusable widgets yet.
 */
/**
 * @file lv_conf.h
